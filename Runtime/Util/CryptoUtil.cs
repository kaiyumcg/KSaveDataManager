...
namespace KSaveDataMan
{
    internal static class CryptoUtil
    {
        ...
        internal static byte[] EncryptIfRequired(string str)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        Debug.LogError("unknown text encoding");
                    }
                }
            }
            return saveBytes;
        ...
        /// <param name="plainText">The text to encrypt.</param>  
        /// <param name="sharedSecret">A password used to generate a key for encryption.</param>  
        internal static string EncryptStringAES(string plainText, bool useAtomicPerDataCredential = false,
            string sharedSecret = "o7x8y6", string salt = "o6806642kbM7c5", EncodingType encodingType = EncodingType.ASCII)
        {
            if (useAtomicPerDataCredential)
            {
                sharedSecret = Config.data.EncryptionConfig.AtomicPerDataSharedSecret;
                salt = Config.data.EncryptionConfig.AtomicPerDataSalt;
                encodingType = Config.data.EncryptionConfig.AtomicPerDataCredentialEncoding;
            }
        ...
        internal static string DecryptStringAES(string cipherText, bool useAtomicPerDataCredential = false,
            string sharedSecret = "o7x8y6", string salt = "o6806642kbM7c5", EncodingType encodingType = EncodingType.ASCII)
        {
            if (useAtomicPerDataCredential)
            {
